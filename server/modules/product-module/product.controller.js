...
exports.getProducts = async (req, res, next) => {
  try {
    ...
    const statement = `SELECT * FROM dau_product`;
    pool.query(statement, (err, result, fileds) => {
      console.log("result", result);
      try {
        if (err) {
...
exports.createProduct = async (req, res, next) => {
  const { body } = req;
  const { name, file_name, short_description, description, price, mrp } = body;

  const getDiscount = () => {
    return (mrp * (100 - price)) / 100;
  };

  const statement = `INSERT INTO dau_product (name, image, short_description, description, price, mrp ) values(
    '${name}',
    '${file_name}',
    '${short_description}',
    '${description}',
...
exports.delete = async (req, res, next) => {
  try {
    ...
    const statement = `DELETE FROM dau_product WHERE id = ${id}`;
